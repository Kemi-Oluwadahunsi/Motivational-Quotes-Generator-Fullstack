    ...
    <div className="grid grid-cols-12 lg:font-medium p-4 lg:px-20 py-3 bg-cyan-900">
      ...
      <section className="col-span-11 grid grid-rows-11 md:flex md:items-center relative">
        ...
        <div className="md:hidden absolute z-50 right-0">
          ...
          >
            {navbar ? (
              <FaTimesCircle className="text-cyan-500 text-md" />
            ) : (
              <GiHamburgerMenu className="text-cyan-500 text-md" />
            )}
          </button>
        </div>
        ...
        {/* Navigation links */}
        <div
          className={` w-full top-0 md:ml-3 items-center gap-4 md:flex md:col-span-4 ${
            navbar
              ? "space-y-2 py-16 absolute top-8 flex flex-col md:flex-row bg-cyan-600"
              : "hidden"
          }`}
        >
          <Link
